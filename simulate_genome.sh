...
if [ "$skip1" == "true" ]; then
	...
	awk '{if(NR==1){print $0} else{print $0"A"}}' ${outdir}/$annotA > temp # add 'A' at the end for later mapping
...
awk '{if(NR==1){print $0} else{print $0"B"}}' ${outdir}/${annotB} > temp  # add 'B' at the end for later mapping
