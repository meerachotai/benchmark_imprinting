...
# use Wyder settings from config file
logfc=${logfc_wyder}
pval=${pval_wyder}
outprefix="roth_wyder"
...
mkdir $outdir/roth_wyder
roth="$outdir/roth_wyder"
...
$scripts_dir/call_imprinting_roth+wyder.R -c ${roth}/${outprefix}_counts.txt -i ${roth}/$outprefix -p $pval -l $logfc -t $cutoff -r $rep ${outdir}/$outprefix
